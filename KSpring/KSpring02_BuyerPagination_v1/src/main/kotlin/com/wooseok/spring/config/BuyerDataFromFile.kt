...
class BuyerDataFromFile(val bRepo: BuyerRepository) {

    /**
     * @Value()
     * *.properties 또는 *.yml 파일에 설정된
     * 속성(com.wooseok.spring.student-file)을 참조하여
     * 속성에 설정된 값(주로 문자열)을 가져와서
     * dataFile 변수에 담아라
     */
    @Value("\${com.wooseok.spring.student-file}")
    private val dataFile: String? = null

    // 00028:길대규:법학:2:8:충청남도 홍성군 홍북면충남대로21:010-5860-6899
    private val 학번 = 0
    private val 이름 = 1
    private val 학과 = 2
    private val 학년 = 3
    ...
    fun dataFromFile():CommandLineRunner {
         ...
         * 파일의 uri 정보를 Paths 객체에 주입하면 Path 객체를 만든다
        ...
        val buyerList = fileList.mapIndexed {index, str:String ->
            ...
            Buyer(
                ...
                address = lines[주소],
                manager = manager[이름],
                manager_tel = manager[전화번호]
            )

        }
        bRepo.saveAll(buyerList)
