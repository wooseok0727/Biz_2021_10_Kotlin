 ...
 * TABLE : Hibernate 가 자체적으로 SEQUENCE TABLE 을 만들고 증가 값을 관리하도록 하기
 * AUTO : 사용하는 DBMS 특성에 따라 SEQ 를 만들거나 Auto_increment 를 생성하여 증가값을 관리
 * 
 * AUTO 설정을 하면 DB 특성에 따라 자체적으로 관리를 하는데
 * 현재는 그렇지 않고 hibernate_sequence 테이블을 생성하여
 * JPA 가 자체적으로 Seq 를 생성하도록 만들어진다
 * auto_increment 가 없는 DB 에서는 AUTO 로 설정하는 것을 권장한다
 */
